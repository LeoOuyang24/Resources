...
public:
    Panel(const glm::vec4& rect_, const glm::vec4& bColor, SpriteWrapper* spr);
    const glm::vec4& getRect();
    void changeRect(const glm::vec4& rect);
    virtual void update(float mouseX,float mouseY, float z, const glm::vec4& scale); //mouseX and mouseY are the coordinates of the mouse, z is the z coordinate to render to,
